...
import {
  ...
  deleteInvites,
  getInvites
} from '../store/Invites/invitesServices';
...
const StyledSearch = styled(AutoComplete)`
 .ant-input-group{
  ...
  .ant-input{
    ...
    &:hover + span > button ,&:focus + span > button{
      z-index: 1075;
    }
  }
...
const Invite = () => {
  ...
  useEffect(() => {
    if(users.users.length){
      const data =users.users.map((user: IUser) =>
        ({ value: user._id ,label: user.name}));
      setOptions(data);
    } else
      setOptions( [{value : '', label: 'No user Found'}]);
  }, [users])
  ...
  const handleOkInviteConfirmation = () => {
    if(optionValue) {
      ...
      dispatch(addInvites(optionValue, () => {
        dispatch(getInvites());
        setValue('');
  ...
  const handleOkDeleteInviteConfirmation = () => {
    if(currentInvites) {
      ...
      dispatch(deleteInvites(currentInvites._id, () => {
        dispatch(getInvites());
            ...
            <Header>
              ...
              <HeaderSearchArea>
                <StyledSearch
                  ...
                  onChange={onChange}
                >
                  <Input.Search size="large" placeholder="Start Typing ...." loading={users.loading} />
                </StyledSearch>
                <InviteButton onClick={() => setVisibleInviteConfirmation(true)}>Invite Staff</InviteButton>
              </HeaderSearchArea>
            </Header>
            <InvitedUserSection>
              {invites.loading ? <Spin/> : invites.invites.map( (invite: IInvites) => <InvitedUser key={invite._id}>
                <p>
                  {invite.user.name}
